...
import { OrderFilters, StatusFilter } from "./filterConfig";
...
function DynamicFilterForm({ onCloseModel }) {
  const { filters, setFilters, clearFilters } = useOrdersFilter();
  ...
  const handleValueChange = (field) => (e) => {
    ...
    setFilters((prev) => {
      const existing = prev.find((f) => f.field === field);
      if (existing) {
        return prev.map((f) => (f.field === field ? { ...f, value: val } : f));
      }
      return [...prev, { field, operator: "is", value: val }];
    });
  };
  ...
  const handleApply = () => {
    // Simply close the modal - filters are applied immediately
    onCloseModel?.();
  };
  ...
  return (
    <FilterContainer>
      <FilterHeader>Filter Orders</FilterHeader>

      {OrderFilters.map(({ field, label, type, options }) => {
